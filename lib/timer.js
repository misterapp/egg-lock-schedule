...
class Timer {
  ...
  handler(key, schedule, listener) {
    ...
    assert(interval || cron || immediate, '[egg-lock-schedule] schedule.interval or schedule.cron or schedule.immediate must be present');
    ...
    if (cron) {
      ...
      try {
        ...
        err.message = `[egg-lock-schedule] parse cron instruction(${cron}) error: ${err.message}`;
        throw err;
      }
  ...
  startCron(key, interval, listener) {
    ...
    try {
      ...
      this.agent.coreLogger.warn(`[egg-lock-schedule] ${msg}`);
      return;
    }
